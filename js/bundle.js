...
g_2:2*gflat,g_3:4*gflat,g_4:8*gflat,g_5:16*gflat,g_6:32*gflat,g_7:64*gflat},duration={ts:.03125,s:.0625,"s-dot":.09375,t:.08333,e:.125,"e-dot":.1875,q:.25,"q-dot":.375,h:.5,"h-dot":.75,w:1,"w-dot":1.5},Offbeat={parse_notes:function(){return this.notes.split(",").map(function(h){return h.trim().split(" ")})},get_time:function(h){return h*this.beatsPerMeasure/(this.tempo/60)},layer:function(h){var k={tempo:60,timeSig:"4/4",instrument:"sine",notes:""};if(h){for(var l in h)if(k.hasOwnProperty(l))k[l]=
h[l];else throw'Property "'+l+'" does not exist';k.beatsPerMeasure=parseInt(k.timeSig,10)}return Object.assign(Object.create(this),k)},generate_audio:function(h,k,l){var n=this,m=h.createOscillator(),p=this.get_time(duration[k[l][0]]),q=frequency[k[l][1]];m.connect(h.destination);m.type=this.instrument;m.frequency.value=q;m.start(0);m.stop(h.currentTime+p);m.onended=function(){l<k.length-1&&!n.isStopped?(l+=1,n.generate_audio(h,k,l)):h.close()}},play:function(){if(""===this.notes)throw'Property "notes" is invalid';
