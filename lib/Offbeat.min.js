...
g_2:2*gflat,g_3:4*gflat,g_4:8*gflat,g_5:16*gflat,g_6:32*gflat,g_7:64*gflat},duration={ts:.03125,s:.0625,"s-dot":.09375,t:.08333,e:.125,"e-dot":.1875,q:.25,"q-dot":.375,h:.5,"h-dot":.75,w:1,"w-dot":1.5},Offbeat={parse_notes:function(h){return h.split(",").map(function(h){return h.trim().split(" ")})},generate_audio:function(h,k){var l=this,m=h.createOscillator(),n=this.calculate_time(duration[this.notesParsed[k][0]]),p=frequency[this.notesParsed[k][1]];m.connect(h.destination);m.type=this.instrument;
m.frequency.value=p;m.start(0);m.stop(h.currentTime+n);m.onended=function(){k<l.notesParsed.length-1&&!l.isStopped?(k+=1,l.generate_audio(h,k)):h.close()}},calculate_time:function(h){return h*this.beatsPerMeasure/(this.tempo/60)},layer:function(h){var k={tempo:60,timeSig:"4/4",instrument:"sine",notes:""};if(h&&"object"===typeof h)for(var l in h)if(k.hasOwnProperty(l)&&typeof k[l]===typeof h[l])k[l]=h[l];else throw Error('Property "'+l+'" is invalid');h=Object.assign(Object.create(this),k);h.beatsPerMeasure=
parseInt(h.timeSig,10);h.notesParsed=this.parse_notes(h.notes);h.isStopped=!1;return h},play:function(){if(""===this.notes)throw Error('Property "notes" is invalid');var h=new (window.AudioContext||window.webkitAudioContext);this.generate_audio(h,0)},playReverse:function(){var h=this;this.notesParsed.reverse();this.play();setTimeout(function(){h.notesParsed.reverse()},1E3*this.time())},playRepeat:function(){var h=this,k=setInterval(function(){h.isStopped?clearInterval(k):h.play()},1E3*this.time())},
playReverseRepeat:function(){var h=this,k=setInterval(function(){h.isStopped?clearInterval(k):h.playReverse()},1E3*this.time())},stop:function(){this.isStopped=!0},time:function(){var h=this;if(""===this.notes)return 0;var k=0;this.notesParsed.forEach(function(l){k+=h.calculate_time(duration[l[0]])});return k}};module.exports=Offbeat;
