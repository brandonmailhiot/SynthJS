...
g_2:2*gflat,g_3:4*gflat,g_4:8*gflat,g_5:16*gflat,g_6:32*gflat,g_7:64*gflat},duration={ts:.03125,s:.0625,"s-dot":.09375,t:.08333,e:.125,"e-dot":.1875,q:.25,"q-dot":.375,h:.5,"h-dot":.75,w:1,"w-dot":1.5},Offbeat={parse_notes:function(h){return h.split(",").map(function(h){return h.trim().split(" ")})},generate_audio:function(h){var k=this,l=this.context.createOscillator(),m=this.calculate_time(duration[this.notesParsed[h][0]]),n=frequency[this.notesParsed[h][1]];l.connect(this.context.destination);
l.type=this.instrument;l.frequency.value=n;l.start(0);l.stop(this.context.currentTime+m);l.onended=function(){h<k.notesParsed.length-1?(h+=1,k.generate_audio(h)):k.ended()}},calculate_time:function(h){return h*this.beatsPerMeasure/(this.tempo/60)},layer:function(h){var k={tempo:60,timeSig:"4/4",instrument:"sine",notes:""};if(h&&"object"===typeof h)for(var l in h)if(k.hasOwnProperty(l)&&typeof k[l]===typeof h[l])k[l]=h[l];else throw Error('Property "'+l+'" is invalid');h=Object.assign(Object.create(this),
k);h.beatsPerMeasure=parseInt(h.timeSig,10);h.notesParsed=this.parse_notes(h.notes);h.context=new (window.AudioContext||window.webkitAudioContext);h.isLoop=!1;return h},update:function(h){if(h&&"object"===typeof h)for(var k in h)if(this.hasOwnProperty(k)&&typeof this[k]===typeof h[k])this[k]=h[k];else throw Error('Property "'+k+'" is invalid');this.beatsPerMeasure=parseInt(this.timeSig,10);this.notesParsed=this.parse_notes(this.notes);return this},play:function(){if(""===this.notes)throw Error('Property "notes" is invalid');
this.generate_audio(0)},playReverse:function(){this.notesParsed.reverse();this.play()},playLoop:function(){this.isLoop=!0;this.play()},playReverseLoop:function(){this.isLoop=!0;this.playReverse()},stop:function(){this.isLoop=!1;this.ended()},time:function(){var h=this;if(""===this.notes)return 0;var k=0;this.notesParsed.forEach(function(l){k+=h.calculate_time(duration[l[0]])});return k},ended:function(){var h=this;this.context.close().then(function(){h.context=new (window.AudioContext||window.webkitAudioContext);
h.isLoop?h.play():h.notesParsed=h.parse_notes(h.notes)})}};module.exports=Offbeat;
